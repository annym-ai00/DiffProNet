...
def train(args, model, train_loader, optimizer, d_head=None):
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        feat_shot = model(data_shot)
        feat_query = model(data_query)

        proto = feat_shot.reshape(args.shot, args.train_way, -1).mean(dim=0)
        ...
        logits = euclidean_metric(feat_query, proto)
        loss = F.cross_entropy(logits, label) + args.beta * ssl_loss
        ...
        proto = None; logits = None; loss = None; feat_shot = None; feat_query = None
